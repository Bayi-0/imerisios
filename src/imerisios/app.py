...
class Imerisios(toga.App):
    def startup(self):
        ...
        self.habit_manage_command = toga.Command(self.open_habit_manage, text="Manage", group=habit_group)
        ...
        # scheduler 
        schedule.every(40).minutes.do(self.day_change)
    ...
    def setup_ui(self, menu=True, settings=True, coin_flip=True, todo=True, habits=True, journal=True):
        # menu
        if menu:
        ...
        if habits:
            ...
            self.habit_tracker_box, self.habit_details_box, self.habit_manage_box, self.habit_more_box = self.habits.setup_habits()
    ...
    def open_habit_manage(self, widget):
        self.habits.clear_habit_manage_box()

        self.main_window.content = self.habit_manage_box
        self.enable_commands()
    ...
    async def restore_databases(self, widget):
        ...
        if result:
            ...
            self.setup_ui(menu=False, settings=False, coin_flip=False, todo=todo, habits=habits, journal=journal)
